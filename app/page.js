...
export default function App() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setProductForm({
          ...
          image: '', composer: '', difficulty: '', genre: '', brand: '', model: ''
        })
      }
    } catch (error) {
  ...
  const filteredProducts = products.filter(product =>
    ...
    (product.composer && product.composer.toLowerCase().includes(searchTerm.toLowerCase())) ||
    ...
    <nav className="border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container flex h-16 items-center justify-between">
        ...
        <div className="flex items-center space-x-1">
          ...
          <Button
            variant={currentPage === 'sheet-music' ? 'default' : 'ghost'}
    ...
    <Card className="h-full flex flex-col">
        ...
        <div className="aspect-square bg-muted rounded-lg mb-4 flex items-center justify-center">
          {product.image ? (
            ...
            <div className="text-muted-foreground">
              {product.category === 'sheet-music' && <Music className="h-12 w-12" />}
              {product.category === 'instruments' && <Guitar className="h-12 w-12" />}
              {product.category === 'accessories' && <Headphones className="h-12 w-12" />}
            </div>
          )}
        </div>
      ...
      <CardContent className="flex-grow">
        <div className="space-y-2">
          ...
          {product.composer && (
            <p className="text-sm text-muted-foreground">Composer: {product.composer}</p>
          )}
          {product.difficulty && (
    ...
    <div className="space-y-12">
      ...
      <section className="relative py-24 bg-gradient-to-br from-primary/10 to-secondary/10">
        <div className="container text-center">
          ...
          <p className="text-xl text-muted-foreground mb-8 max-w-2xl mx-auto">
            Discover thousands of sheet music pieces, quality instruments, and professional accessories 
          ...
          <div className="flex gap-4 justify-center flex-wrap">
            ...
            <Button size="lg" variant="outline" onClick={() => setCurrentPage('equipment')}>
              Shop Instruments
            </Button>
          </div>
        </div>
      ...
      <section className="container">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => setCurrentPage('sheet-music')}>
          ...
          <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => setCurrentPage('equipment')}>
            <CardHeader className="text-center">
              <Guitar className="h-16 w-16 mx-auto mb-4 text-primary" />
              <CardTitle>Instruments</CardTitle>
              <CardDescription>Guitars, Pianos, Violins, and more</CardDescription>
            </CardHeader>
          </Card>
          <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => setCurrentPage('equipment')}>
            <CardHeader className="text-center">
    ...
    <div className="container py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-muted-foreground">Discover thousands of sheet music pieces for all skill levels</p>
        </div>
        <div className="flex items-center space-x-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {getProductsByCategory('sheet-music').map(product => (
          <ProductCard key={product.id} product={product} showActions={isAdmin} />
        ))}
      </div>
    </div>
  )
    ...
    <div className="container py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-muted-foreground">Professional instruments and accessories for musicians</p>
        </div>
        <div className="flex items-center space-x-4">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground h-4 w-4" />
  ...
  const ServicesPage = () => (
    <div className="container py-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Digital tools and services to enhance your musical journey</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Interactive lessons and practice tools</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>One-on-one instruction with professional teachers</CardDescription>
          </CardHeader>
          <CardContent>
            <ul className="space-y-2 text-sm">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Professional maintenance and repair services</CardDescription>
          </CardHeader>
          <CardContent>
            <ul className="space-y-2 text-sm">
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-muted-foreground">
                Authentication is not enabled yet. In the future, you'll be able to:
              </p>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center space-x-4">
                      <div className="w-16 h-16 bg-muted rounded flex items-center justify-center">
                        {item.category === 'sheet-music' && <Music className="h-8 w-8" />}
                        {item.category === 'instruments' && <Guitar className="h-8 w-8" />}
                        {item.category === 'accessories' && <Headphones className="h-8 w-8" />}
                      </div>
    ...
    <Dialog open={showProductDialog} onOpenChange={setShowProductDialog}>
      <DialogContent className="max-w-2xl">
        ...
        <form onSubmit={handleProductSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setProductForm({...productForm, composer: e.target.value})}
                />
              </div>
              <div>
                <Label htmlFor="difficulty">Difficulty</Label>
                <Select value={productForm.difficulty} onValueChange={(value) => setProductForm({...productForm, difficulty: value})}>
                  <SelectTrigger>
          ...
          {(productForm.category === 'instruments' || productForm.category === 'accessories') && (
            <div className="grid grid-cols-2 gap-4">
              <div>
  ...
  const renderCurrentPage = () => {
    switch (currentPage) {
      ...
      case 'sheet-music': return <SheetMusicPage />
